...
import { dcuplKeys } from '@secrets/dcupl-keys';
import {
  BehaviorSubject,
  Observable,
  defer,
...
export function injectDcupl$() {
  ...
  return Object.assign(new Observable<Dcupl<string>>(), {
    ...dcupl$,
    readyMap: <T>(fn: (dcupl: Dcupl<string>) => T) => {
      return dcupl$.pipe(map((dcupl) => fn(dcupl)));
    },
  });
}
